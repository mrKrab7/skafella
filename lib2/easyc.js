...
EasyC.prototype.draw = function (success, i, sorted) {
    ...
    if (obj.relative.coord.x == true) {
        coord.x *= this.canvas.clientWidth;
    }
    if (obj.relative.coord.y == true) {
        coord.y *= this.canvas.clientHeight;
    }
    var scale = [1, 1];
    if (obj.scale) {
    ...
    if (obj.type == "circle") {
        ...
        if (obj.width) {
            width = obj.width;
            if (obj.relative.size.width == true) {
                width *= this.canvas.clientWidth;
            }
        }
        if (obj.height) {
            height = obj.height;
            if (obj.relative.size.height == true) {
        ...
        if (obj.crop) {
            crop = obj.crop;
            if (obj.relative.crop.coord.x == true) {
                crop[0] *= img.naturalWidth;
            }
            if (obj.relative.crop.coord.y == true) {
                crop[1] *= img.naturalHeight;
            }
            if (obj.relative.crop.size.width == true) {
                crop[2] *= img.naturalWidth;
            }
            if (obj.relative.crop.size.height == true) {
                crop[3] *= img.naturalHeight;
            }
        }
...
EasyC.prototype.getSizeFromPoints = function (points) {
    ...
    for (var i = 0; i < points.length; i++) {
        if (intervalX[0] == null || points[i][0] < intervalX[0]) {
            intervalX[0] = points[i][0];
        }
        if (intervalX[1] == null || points[i][0] > intervalX[1]) {
            intervalX[1] = points[i][0];
        }
        if (intervalY[0] == null || points[i][1] < intervalY[0]) {
            intervalY[0] = points[i][1];
        }
        if (intervalY[1] == null || points[i][1] > intervalY[1]) {
            intervalY[1] = points[i][1];
        }
    }
